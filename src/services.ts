...
export const user_by_id = async (id: string) => {
  ...
  return await users.findOne<
    AuthUserModel | Promise<AuthUserModel | undefined> | undefined
  >({ id: id });
};
...
const classesFull = async (classes: ClassType[]): Promise<ClassTypeFull[]> => {
  ...
  const studentsMap = new Map(
    studentsData.map((s) => [s.id, s as unknown as Student])
  );
...
export const class_filter = async (
  filter: {
    name?: string;
    page?: number;
    sortBy?: keyof ClassType;
    order?: "asc" | "desc";
  },
  all = false
) => {
  ...
  const totalResult = await classes.countDocuments(query);

  let cursor = classes.find(query);

  if (sortBy) {
    cursor = cursor.sort({ [sortBy]: _order === "desc" ? -1 : 1 });
  }

  if (!all) {
    cursor = cursor.skip((_page - 1) * rowsPerPage).limit(rowsPerPage);
  }
    ...
    await classesFull([
      {
        ...
        studentIds: classData.studentIds as string[],
      },
    ])
  )[0];
};
...
export const class_update = async (classs: ClassType) => {
  ...
  const classes = db.collection("classes");
  await classes.updateOne({ id: classs.id }, { $set: classs });
  return classs;
};
...
const subjectsFull = async (subjects: Subject[]): Promise<SubjectFull[]> => {
  ...
  const teachersMap = new Map(
    teacherData.map((t) => [t.id, t as unknown as Teacher])
  );
  const classesMap = new Map(
    classData.map((c) => [c.id, c as unknown as ClassType])
  );
  ...
  return subjects.map((subject) => {
    ...
    if (!teacher || !classData) {
      throw new Error(
        `Missing teacher or class data for subject ${subject.id}`
      );
    }
...
export const subject_filter = async (
  filter: {
    name?: string;
    classIds?: string[];
    schedule?: string;
  ...
  if (schedule) {
    query.daysOfWeek = schedule.toLowerCase();
  }

  const totalResult = await subjects.countDocuments(query);

  let cursor = subjects.find(query);
...
export const subject_update = async (subject: Subject) => {
  ...
  const subjects = db.collection("subjects");
  await subjects.updateOne({ id: subject.id }, { $set: subject });
  const result = await subjectsFull([subject]);
  return result[0];
};
...
export const teacher_filter = async (
  filter: {
    name?: string;
    classIds?: string[];
    page?: number;
  ...
  if (classIds && classIds.length) {
    const subjects = db.collection("subjects");
    const subjectData = await subjects
  ...
  const totalResult = await teachers.countDocuments(query);

  let cursor = teachers.find(query);
...
export const teacher_update = async (teacher: Teacher) => {
  ...
  const teachers = db.collection("teachers");
  await teachers.updateOne({ id: teacher.id }, { $set: teacher });
  return teacher;
};
...
export const parent_filter = async (
  filter: {
    name?: string;
    classIds?: string[];
    page?: number;
  ...
  if (classIds && classIds.length) {
    ...
    const studentData = await students
      .find({ classIds: { $in: classIds } })
      .toArray();
    const parentIds = studentData.flatMap((student) => student.parentIds);
    query.id = { $in: parentIds };
  }

  const totalResult = await parents.countDocuments(query);

  let cursor = parents.find(query);
...
export const parent_update = async (parent: Parent) => {
  ...
  const parents = db.collection("parents");
  await parents.updateOne({ id: parent.id }, { $set: parent });
  return parent;
};
...
const studentsFull = async (students: Student[]): Promise<StudenFull[]> => {
  ...
  const parentsMap = new Map(
    parentData.map((p) => [p.id, p as unknown as Parent])
  );
  const classesMap = new Map(
    classData.map((c) => [c.id, c as unknown as ClassType])
  );
...
export const student_filter = async (
  filter: {
    name?: string;
    classIds?: string[];
    page?: number;
  ...
  const totalResult = await students.countDocuments(query);

  let cursor = students.find(query);
...
export const student_update = async (student: Student) => {
  ...
  const students = db.collection("students");
  await students.updateOne({ id: student.id }, { $set: student });
  const result = await studentsFull([student]);
  return result[0];
};
...
export const db_stats = async () => {
  ...
  const [studentCount, parentCount, teacherCount, classCount] =
    await Promise.all([
  ...
  const genderStats = await students
    .aggregate([
      {
        $group: {
          _id: "$gender",
  ...
  const femaleCount =
    genderStats.find((stat) => stat._id === "female")?.count || 0;

  // Get enrollments per year using MongoDB's aggregation
  const enrollmentsPerYear = await students
    .aggregate([
      {
        $group: {
  ...
  return {
    ...
    teacherCount,
    classCount,
    enrollmentsPerYear: enrollmentsPerYear.map((stat) => [
      stat.year,
      stat.count,
    ]),
  };
